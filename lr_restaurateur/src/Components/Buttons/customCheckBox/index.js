...
const UseCheck = ({item,HandelChangeCheck}) => {
  return (
    <TouchableOpacity style={{flexDirection:"row",width:"90%" ,justifyContent:"space-between",marginVertical:7,
    marginHorizontal:20 }}  
    
    onPress={()=>{HandelChangeCheck(item.id)}}
    >
      <Txt fontSize={16} color={item.color}>{item.name}</Txt>
